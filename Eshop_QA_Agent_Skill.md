...
2. **No Hallucination:** Hệ thống EShop SUT **KHÔNG có chức năng quản lý tồn kho (stock)** và **KHÔNG có tính năng xóa sản phẩm khỏi giỏ hàng khi stock bằng 0**. Môi trường test là web/mobile thuần. Chỉ thiết kế test case dựa trên đặc tả được cung cấp.
...
4. **Trạng thái mặc định:** Cột "Kết quả thực tế (Actual Result)" luôn để trống. Cột "Verdict" luôn điền "N/A".

---

# STRICT OUTPUT SPECIFICATION (TECHNICAL WRITER OUTPUT)

Bạn phải tổng hợp kết quả và đóng gói thành **3 khối văn bản riêng biệt đại diện cho 3 file**. Mỗi khối bắt đầu bằng định dạng: `[File: FR-XX_filename.md]`. Sau đó tạo ra 3 file với tên và nội dung tương ứng trực tiếp vào trong môi trường hiện tại.
...
(Viết phần giải thích chi tiết, step-by-step cách phân tích các biến đầu vào, biến trạng thái. Liệt kê rõ Miền hợp lệ - Valid Domains và Miền không hợp lệ - Invalid Domains). Tất cả dưới dạng gạch đầu dòng mỗi thuộc tính để dễ đọc, đồng thời gán các kí hiệu miền phù hợp để phân biệt (ví dụ: E1, E2,...)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| (Data) | (Data) | (Chỉ cần tóm gọn cần làm gì) | (Sử dụng `<br>` để ngắt dòng) | Chưa thực hiện | | N/A |

### Boundary Value Analysis (BVA)

**Phân tích các giá trị biên (Step-by-step Explanation):**
(Viết phần phân tích các ranh giới min, max, just below, just above, on the boundary từ đặc tả). Tất cả dưới dạng gạch đầu dòng mỗi thuộc tính để dễ đọc, đồng thời gán các kí hiệu ranh giới phù hợp để phân biệt (ví dụ: B1, B2,...)
...
- **Ảnh minh chứng:**
  ![Bug Image](images/bug_XX.png)

_(Đây là template để tester copy và log bug lên github issue)_
